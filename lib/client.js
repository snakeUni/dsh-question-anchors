	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(CSS_ID) + "]") === null) {
			...
			tag.textContent = [
				...
				".dsh-qa-pill{box-sizing:border-box;height:auto;min-height:44px;width:40px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:var(--dsw-specific-menu);border:1px solid var(--dsw-alias-border-l2);border-radius:999px;box-shadow:var(--dsw-shadow-lv3);flex-direction:column;align-items:center;justify-content:center;gap:5px;padding:8px 0;display:flex;transition:color .15s, border-color .15s, transform .15s}",
				...
				".dsh-qa-anchorIcon{width:14px;height:14px;flex:none}",
				".dsh-qa-card{box-sizing:border-box;width:252px;max-height:calc(100vh - 120px);background:var(--dsw-specific-menu);border:1px solid var(--dsw-alias-border-l2);border-radius:14px;box-shadow:var(--dsw-shadow-lv3);flex-direction:column;display:flex;overflow:hidden;--dsh-scrollbar-thumb:var(--dsw-alias-scrollbar-bg-l2);--dsh-scrollbar-thumb-hover:var(--dsw-alias-scrollbar-hover-l2)}",
				".dsh-qa-header{box-sizing:border-box;flex:none;align-items:center;gap:8px;padding:10px 8px 10px 14px;display:flex;border-bottom:1px solid var(--dsw-alias-border-l2)}",
				".dsh-qa-title{min-width:0;color:var(--dsw-alias-label-primary);flex:1;font-size:13px;font-weight:600;line-height:20px;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;display:flex;align-items:center;gap:6px}",
				".dsh-qa-title svg{flex:none;color:var(--dsw-alias-state-business-primary)}",
				".dsh-qa-count{flex:none;min-width:20px;height:18px;color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-state-business-tertiary);border-radius:999px;align-items:center;justify-content:center;padding:0 6px;font-size:11px;line-height:18px;font-variant-numeric:tabular-nums;display:flex}",
				...
				".dsh-qa-item{box-sizing:border-box;width:100%;min-height:40px;color:var(--dsw-alias-label-secondary);text-align:left;cursor:pointer;background:0 0;border:none;border-radius:9px;align-items:center;gap:8px;padding:6px 8px;display:flex;transition:background .12s, color .12s;position:relative}",
				".dsh-qa-item:hover{background:var(--dsw-alias-interactive-bg-hover)}",
				".dsh-qa-itemActive{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-state-business-tertiary)}",
				".dsh-qa-itemActive::before{content:\"\";position:absolute;left:0;top:9px;bottom:9px;width:3px;border-radius:2px;background:var(--dsw-alias-state-business-primary)}",
				".dsh-qa-itemActive .dsh-qa-itemText{font-weight:600}",
				".dsh-qa-itemActive .dsh-qa-itemIndex{background:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-label-primary-foreground);font-weight:700}",
				".dsh-qa-itemIndex{flex:none;min-width:18px;height:18px;color:var(--dsw-alias-label-caption);border-radius:5px;align-items:center;justify-content:center;font-size:11px;line-height:18px;font-variant-numeric:tabular-nums;display:flex;transition:color .12s}",
				...
				".dsh-qa-foot{flex:none;color:var(--dsw-alias-label-caption);border-top:1px solid var(--dsw-alias-border-l2);padding:6px 14px;font-size:11px;line-height:16px}",
		...
		const QuestionAnchorsPanel = (props) => {
			...
			/* Jump lock: while a click-initiated smooth scroll is in flight, the
			 * scroll-spy must not re-highlight intermediate anchors (they flash
			 * past during the scroll). The lock holds the jump target until the
			 * scroller reaches it (or a timeout backstop releases it). */
			const jumpLockRef = react.useRef(null);
			...
			react.useEffect(() => {
				...
				const update = () => {
					raf = 0;
					/* While a jump lock is held, keep the click target
					 * highlighted and release once the scroller arrives. */
					const lock = jumpLockRef.current;
					if (lock !== null) {
						setActive(lock.key);
						const row = rowOf(lock.key);
						if (row) {
							const targetTop = Math.max(flowTop(row, scroller) - 14, 0);
							if (Math.abs(scroller.scrollTop - targetTop) < 6) {
			...
			const scrollToAnchor = react.useCallback((key) => {
				...
				setActive(key);
				/* Hold the click target against scroll-spy interference until the
				 * smooth scroll arrives (the spy releases it via `update`), with
				 * a timeout backstop so a cancelled/interrupted scroll cannot
				 * strand the lock. */
			...
			react.useEffect(() => () => {
				...
				if (jumpLockRef.current !== null) clearTimeout(jumpLockRef.current.timer);
			}, []);
					...
					children: react_jsx_runtime.jsxs("button", {
						...
						children: [react_jsx_runtime.jsx("svg", {
							...
							children: [react_jsx_runtime.jsx("path", {
								d: "M8 1.5c3.6 0 6.5 2.6 6.5 5.8 0 1.4-.5 2.7-1.3 3.8l.6 3-2.9-1.5c-.9.4-1.9.5-2.9.5-3.6 0-6.5-2.6-6.5-5.8S4.4 1.5 8 1.5Z",
								stroke: "currentColor",
								strokeWidth: 1.4,
